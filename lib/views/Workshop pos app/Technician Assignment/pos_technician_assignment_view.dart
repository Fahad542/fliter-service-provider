...
// ── Localisation helper ───────────────────────────────────────────────────────
// Called inside a build method so that it re-evaluates whenever the locale
// changes — the Duration is recomputed on every rebuild, so there is no
// stale-locale issue.

/// Returns a translated "last seen" string for [tech].
/// Must be called from a build method (requires a valid [BuildContext]).
String localizedLastSeen(BuildContext context, PosTechnician tech) {
  final l10n = AppLocalizations.of(context)!;
  final dur = tech.lastSeenDuration;
  if (dur == null) return l10n.posTechLastSeenNever;
  ...
  return tech.formattedLastSeenDate; // YYYY-MM-DD — numerals are locale-agnostic
}
...
/// [context.read] for the assignment VM is valid (parent [State] context is
/// above the provider).
...
class _AssignmentCatalogBootstrapState
    extends State<_AssignmentCatalogBootstrap> {
  @override
  void initState() {
    super.initState();
...
class _PosTechnicianAssignmentViewState
  ...
  bool get _canTapBroadcast => widget.jobId.isNotEmpty || widget.isWalkIn;
  ...
  void _handleMultiAssign(
    ...
    try {
      if (widget.isWalkIn) {
        ...
        if (isEditMode) {
          ...
          if (existingJobId.isEmpty) {
            ToastService.showError(context, l10n.posTechAssignFailEditId);
            return;
          }
          ...
          if (jobIdToUse.isEmpty) {
            ToastService.showError(context, l10n.posTechAssignFailGetId);
            return;
          }
        }
      }
      ...
      if (jobIdToUse.trim().isEmpty) {
        ToastService.showError(context, l10n.posTechAssignFailNoJob);
        return;
      }

      // Snapshot the pre-assign job for completed-unlock logic
      ...
      if (!widget.isWalkIn && jobSnapshotPreAssign != null) {
        ...
        final st = PosViewModel.normalizeCashierJobStatus(
            jobSnapshotPreAssign.status);
        if (techsChanged && st == 'completed') {
          final oid =
          (posVm.selectedOrder?.id ?? posVm.editingOrder?.id)?.trim();
          if (oid != null && oid.isNotEmpty) {
            final marked =
            await posVm.tryMarkCashierJobEditedAfterMeaningfulChange(
              context,
              jobId: jobIdToUse,
              orderId: oid,
            ...
            if (!marked) {
              ToastService.showError(
                context,
                posVm.errorMessage ?? l10n.posTechAssignUnlockFail,
              );
              return;
            }
          }
      ...
      if (success) {
        ...
        if (assignVm.selectedTechnicianIds.isEmpty) {
          ...
          final fromSelection = _jobTechniciansFromSelection(
              assignVm.selectedTechnicianIds, vm.rawTechnicians);
          final fromServer = vm.lastCashierAssignTechnicians;
          techs = (fromServer.isNotEmpty &&
              fromServer.length == fromSelection.length)
              ? List<JobTechnician>.from(fromServer)
              : fromSelection;
        }

        if (oid != null && oid.isNotEmpty) {
          posVm.applyJobTechniciansFromAssign(
              orderId: oid, jobId: jobIdToUse, technicians: techs);
        }
        ...
        ToastService.showSuccess(
          ...
          serverMsg.isNotEmpty
              ...
              ? l10n.posTechAssignSuccessEmpty
              : l10n.posTechAssignSuccess),
        );
        ...
        if (oid != null && oid.isNotEmpty) {
          ...
          Future<void>.delayed(const Duration(milliseconds: 550), () {
            posVm.applyJobTechniciansFromAssign(
                orderId: oidDelayed, jobId: jid, technicians: techsDelayed);
          });
        }
        ...
        ToastService.showError(
          context,
          vm.assignmentMessage ?? l10n.posTechAssignFailGeneric,
        );
      }
    } finally {
  ...
  Future<String?> _resolveJobIdForBroadcast(BuildContext context) async {
    ...
    if (isEditMode) {
      ...
      final success = await posVm.submitEditOrder(
          deptId.isNotEmpty ? [deptId] : [], context);
                ...
                children: [
                  ...
                  Consumer<TechnicianViewModel>(
                    builder: (context, vm, _) {
                      final listLoading = vm.isLoading && !vm.hasTechnicianList;
                      ...
                      if (vm.errorMessage != null &&
                        ...
                        return Center(
                          child: Column(
                            ...
                            children: [
                              Text(l10n.posTechAssignErrorPrefix(
                                  vm.errorMessage!)),
                              ...
                              ElevatedButton(
                                ...
                                child: Text(l10n.posTechAssignRetry),
                              ),
                            ],
                          ),
                        );
                      ...
                      bool passesDeptAndOnline(PosTechnician tech) {
                        ...
                        if (deptName.isNotEmpty) {
                          final inDept = tech.departments.any((d) =>
                      ...
                      // Selected techs always visible; then filtered unselected
                                  ...
                                  child: Container(
                                    ...
                                    child: TextField(
                                      ...
                                      decoration: InputDecoration(
                                        hintText: l10n.posTechAssignSearchHint,
                                        hintStyle: TextStyle(
                              ...
                              GestureDetector(
                                ...
                                child: Padding(
                                  ...
                                  child: Container(
                                    ...
                                    // Flexible text prevents overflow in Arabic
                                    child: Text(
                                      assignVm.onlineOnly
                                          ? l10n.posTechAssignShowAll
                                          : l10n.posTechAssignOnlineOnly,
                                          ...
                                          Expanded(
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                // build so locale changes
                                                // are reflected immediately.
                                                Text(
                                                  tech.isOnline
                                                      ? l10n
                                                      .posTechAssignStatusOnline
                                                      : l10n.posTechAssignStatusLastSeen(
                                                      localizedLastSeen(
                                                          context,
                                                          tech)),
                                                  style: TextStyle(
                                                ...
                                                const SizedBox(
                                                    height: 3),
                                                // Slots — translated via parameterised key;
                    ...
                    children: [
                      Consumer<TechnicianViewModel>(
                        builder: (context, vm, _) {
                          ...
                          final cooldownLabel =
                          broadcastCooldown && cooldownKey.isNotEmpty
                              ? posVm.broadcastCooldownTimerLabelForKey(
                              cooldownKey)
                              : '';

                          return Row(
                            children: [
                              // Broadcast button (only for broadcastable jobs)
                              if (_canTapBroadcast) ...[
                                Expanded(
                                  child: SizedBox(
                                    height: 50,
